...
class WM811KExtended(Dataset):
    ...
    def __init__(self, root, transforms=None, proportion=1.0, decouple_input: bool = True, **kwargs):
        super(WM811KExtended, self).__init__()
    ...
    def __getitem__(self, idx):
        idx_transform = idx // len(self.samples)
        idx_sample = idx % len(self.samples)

        path, y = self.samples[idx_sample]
        x = self.load_image_cv2(path)

        if self.transforms[idx_transform] is not None:
            x = self.transforms[idx_transform](x)
    ...
    def __len__(self):
        return len(self.samples) * self.n_transforms
